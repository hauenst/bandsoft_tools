	...
	while(reader.next()==true){
		...
		for( it = pmts_adc.begin() ; it != pmts_adc.end() ; ++it){
			...
			if( it->second.size() == 1 && pmts_tdc[PMT_ID].size() == 1){
				...
				if( iteration >= 1 ){
					tdc_ampcorr -= ( TWParamsAMP_It1[PMT_ID][0] + TWParamsAMP_It1[PMT_ID][1] / pow(amp,TWParamsAMP_It1[PMT_ID][2]) );  // first iteration
					tdc_adccorr -= ( TWParamsADC_It1[PMT_ID][0] + TWParamsADC_It1[PMT_ID][1] / pow(adc,TWParamsADC_It1[PMT_ID][2]) );  // first iteration
...
void fitTW(TH2D * hist , TCanvas * c, int cd, int s, int l, int co, int o, double cut, 
	...
	TFitResultPtr ptr = g->Fit(model,"QESBR","",0,1200);
...
void walkCorr(	vector<double> *adcs		,
	...
	while( currBin < maxBin ){
		...
		int nEvents = 1000;
		if( currBin_x > 1000 ) nEvents = 8000;	// first iteration
