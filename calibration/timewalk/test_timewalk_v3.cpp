	...
	while(reader.next()==true){
		...
		for( it = pmts_adc.begin() ; it != pmts_adc.end() ; ++it){
			...
			if( it->second.size() == 1 && pmts_tdc[PMT_ID].size() == 1){
				...
				if( iteration >= 2 ){
					tdc_ampcorr -= ( TWParamsAMP_It1[PMT_ID][0] + TWParamsAMP_It1[PMT_ID][1] * amp ) ; // second iteration
					tdc_adccorr -= ( TWParamsADC_It1[PMT_ID][0] + TWParamsADC_It1[PMT_ID][1] * adc ) ; // second iteration
				}
				if( iteration >= 3 ){
					if( TWParamsAMP_It2[PMT_ID][0] != 0 && period == 1)
						tdc_ampcorr -= ( TWParamsAMP_It2[PMT_ID][0]*sin( (amp-TWParamsAMP_It2[PMT_ID][1])/TWParamsAMP_It2[PMT_ID][2] ) + TWParamsAMP_It2[PMT_ID][3]/pow(amp,TWParamsAMP_It2[PMT_ID][4]) );
